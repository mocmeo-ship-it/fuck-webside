...
def max_damage():
    ...
    parser.add_argument('--number', type=int, default=2000, help='Số lượng request muốn gửi')
    parser.add_argument('--concurrency', type=int, default=20, help='Số lượng request đồng thời muốn gửi')
    ...
    parser = argparse.ArgumentParser(description='Xin chào! Bạn muốn làm gì?')
    parser.add_argument('--option', type=str, choices=options, help='Chọn một trong các tùy chọn sau đây: ' + ', '.join(options), required=True)
    args = parser.parse_args()

    if args.option == 'HTTP':
        parser = argparse.ArgumentParser(description='HTTP')
        parser.add_argument('url', type=str, help='URL để gửi request')
        parser.add_argument('--header', type=str, help='Header(s), phân biệt bởi dấu xuống dòng')
